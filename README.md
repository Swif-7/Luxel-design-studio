...
预览、播放栏和四类流动的选择条组成固定预览区：常见桌面尺寸下整块不用滚动就能看全，往下滚时整体吸附，参数继续滚动，调参和换形态都不必来回翻页。宽窗口中预览固定在左侧；窄窗口中预览高度约占视口三成、选择条改为横向滑动；横屏等高度不足 560px 时取消吸附，按普通顺序滚动。参数面板在铺满整页宽度时自动把滑块和色标分成多列，不会拉成整屏宽的一行。
