...
- 预览工具栏的「导出」菜单提供 PNG、参数 JSON、独立 HTML 三项。PNG 不含页面文字；JSON 保存完整设置，可从「随机与种子 → 导入参数」恢复；HTML 内置当前参数、动画进度、播放状态、原创渲染代码与 MIT 许可，不引用本地服务或外部资源，提供播放／暂停按钮。HTML 按打开窗口的比例铺满画面，因此不同窗口比例会改变可见构图。文件名为 `rheo-<种子哈希>.png` / `.html` 和 `rheo-parameters.json`。底部说明区已移除，自动保存仍保留。
