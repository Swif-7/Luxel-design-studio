import {defaults,modes,palettes,hashSeed,generate,validate,randomizePalette,pinkStudy} from './model.js';
import {glyphGroups,glyphsFor} from './glyphs.js';
import {Renderer} from './shader.js';
const $=id=>document.getElementById(id);let state={...defaults,colors:[...defaults.colors]};let elapsed=0,paused=matchMedia('(prefers-reduced-motion: reduce)').matches,last=0,frames=0,fpsStart=0,renderer,toastTimer,lost=false;
...
const hex=state.background.slice(1);const bgLuma=.2126*parseInt(hex.slice(0,2),16)+.7152*parseInt(hex.slice(2,4),16)+.0722*parseInt(hex.slice(4,6),16);$('stage').classList.toggle('light-canvas',bgLuma>150);for(const [key] of controls)$(key+'-value').textContent=(key==='density'||key==='blur')?Math.round(state[key])+'%':state[key].toFixed(key==='angle'?0:2)+(key==='speed'?'×':key==='angle'?'°':'');$('background-hex').textContent=state.background;$('effect-name').textContent=modes[state.mode];$('seed-caption').textContent='SEED / '+state.seed;$('particle-controls').classList.toggle('disabled',!state.particles);for(const el of $('particle-controls').querySelectorAll('input,select'))el.disabled=!state.particles;document.querySelectorAll('.preset').forEach((el,i)=>{el.classList.toggle('active',i===state.mode);el.setAttribute('aria-pressed',i===state.mode);});$('pause').textContent=paused?'▶':'Ⅱ';$('pause').setAttribute('aria-label',paused?'播放动画':'暂停动画');if(!lost)draw();}
for(let i=0;i<modes.length;i++){const button=document.createElement('button');button.className='preset';button.setAttribute('aria-label',modes[i]);button.innerHTML=`<img alt="" width="180" height="120"><span class="preset-label">${modes[i].split(' · ')[0]}<small>${String(i+1).padStart(2,'0')}</small></span>`;button.onclick=()=>{state.mode=i;elapsed=0;sync();save();};$('presets').append(button);}
...
palettes.forEach((p,i)=>{const b=document.createElement('button');b.className='palette';b.style.background=`linear-gradient(110deg,${p.slice(1).join(',')})`;b.setAttribute('aria-label',['虹彩','清透青','落日','紫晶','深海'][i]+'配色');b.onclick=()=>{state.background=p[0];state.colors=p.slice(1);[state.primary,state.accent]=state.colors;sync();save();};$('palettes').append(b);});
$('pink-study').onclick=()=>{state={...pinkStudy,colors:[...pinkStudy.colors],quality:state.quality,lockColors:state.lockColors,lockMode:state.lockMode};elapsed=0;sync();save();toast('已应用粉霞构图：同源渐变背景与近景色带');};
...
try{const c=document.createElement('canvas');const preview=new Renderer(c);document.querySelectorAll('.preset img').forEach((img,i)=>{preview.draw({...defaults,mode:i,seedValue:hashSeed(defaults.seed)%10000,particles:false},3,240,150);img.src=c.toDataURL();});preview.destroy();}catch{}
function frame(now){const delta=last?Math.min((now-last)/1000,.1):0;last=now;if(!document.hidden&&!lost){if(!paused)elapsed+=delta*state.speed;draw();frames++;if(now-fpsStart>1000){$('fps').textContent=Math.round(frames*1000/(now-fpsStart))+' FPS';frames=0;fpsStart=now;$('resolution').textContent=$('canvas').width+' × '+$('canvas').height;}const seconds=Math.floor(elapsed);$('time').textContent=String(Math.floor(seconds/60)).padStart(2,'0')+':'+String(seconds%60).padStart(2,'0');}requestAnimationFrame(frame);}requestAnimationFrame(frame);
